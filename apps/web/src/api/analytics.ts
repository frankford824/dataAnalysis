...
type OverviewResponse = { metrics: Record<string, string | number> }
...
export async function getOverview(filters: FilterState): Promise<DashboardSummary> {
  ...
  const storeRows = new Map<string, { revenue: number; refund: number; fees: number; cost: number; profit: number }>()
  for (const row of exported.rows) {
    ...
    const current = storeRows.get(storeId) || { revenue: 0, refund: 0, fees: 0, cost: 0, profit: 0 }
  ...
  return {
    revenue: metric('revenue'), refund: metric('refund'), fees: metric('fees'), cost: totalCost, profit: metric('profit'),

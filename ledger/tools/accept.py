...
#:
#: 只剩一条：商家集运物流责任货值赔付改判交易赔付（原先是物流运费）。人工表里那一列
#: 物流运费 -363.79 引擎不再出——费项分类对照表五个平台加起来没有「物流运费」这个
#: 业务大类，唯一撑着它的费项已经改判走了。
#:
#: 两个数都能逐项拆平：
#:   交易赔付 +319.41 —— 集运货值赔付那 31 行搬进来且不再取反，人工表还记在物流运费里；
#:   利润     +939.52 —— 上面的 319.41，加上物流运费那一行退出的 363.79，
#:                       再加上已登记的数据版本差异之和 256.31（销售收入 -561.28、
#:                       聚水潭成本 +675.85、软件服务费 +52.38、营销费用 +27.66、
#:                       发货运费 +27.41、刷单 +37.20、客服打款 +10.00、销售退款 -12.91）。
#:
#: 这里原先还登记着一条「保证金锁定改判交易赔付」，认的是 -1,036.86。那句话说错了对象：
#: 它数的那 107 行不是退款锁定（这份表里根本没有那个科目），是保证金-天猫-出账缴存里
#: 备注恰好等于「天猫保证金-充值（代扣）」的行——纯充值，对账表公式说明的条件 8 明写
#: 要把费项清空，人工表也确实清着。引擎当时把整个科目当成交易赔付算了进去，
#: 是错的而不是新口径。2026-08-17 按备注精确匹配排掉之后，交易赔付上的差从 -997.75
#: 收到 +319.41，剩下的正好只有集运这一条。
#:
#: 同一轮还修掉了记账本转账取不到订单号（业务基础订单号那一格放的是打款流水号
#: FP301_***，真订单号在备注的「关联订单号：」后面）。人工表算这 32 行 -214.87
#: ——它自己那张对账表的费项2 列写着交易赔付。原先登记里那句「交易赔付 +39.11
#: 是数据版本差异」，其实就是这批行漏算，金额一分不差。
_TAOBAO_RECLASSIFIED = (
    "业务在 2026-08 把商家集运物流责任货值赔付从物流运费改判成交易赔付，人工表还是"
    "老口径。详见 _TAOBAO_RECLASSIFIED 上方的逐笔拆解。"
)
...
CASES = {
    "淘宝喜必顺": Case(
        ...
        explained={
            ...
            "交易赔付": (319.41, _TAOBAO_RECLASSIFIED),
            "利润": (939.52, _TAOBAO_RECLASSIFIED),
            "聚水潭成本": (675.85,

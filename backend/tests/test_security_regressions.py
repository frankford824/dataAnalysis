...
from app.standard_model import registry_checksum
...
def test_openapi_documents_cookie_only_authentication(client):
    ...
    assert schema["paths"]["/api/v1/stores"]["get"]["security"] == [{"SessionCookie": []}]


def test_expired_cookie_session_is_rejected(client):
...
def test_email_is_globally_unique_and_store_scope_is_enforced(client, tenants):
    ...
    allowed = client.post(
        ...
        json={"name": "Allowed store", "activation_at": "2026-01-01T00:00:00Z", "platform_account_id": platform["id"]},
    ).json()
    denied = client.post(
        "/api/v1/stores",
        headers=tenant_headers(first),
        json={"name": "Denied store", "activation_at": "2026-01-01T00:00:00Z", "platform_account_id": platform["id"]},
    ).json()
    invite = client.post(
        "/api/v1/users/invite",
        headers=tenant_headers(first),
    ...
    visible = client.get("/api/v1/stores")
    assert visible.status_code == 200
    assert [item["logical_id"] for item in visible.json()] == [allowed["logical_id"]]
    assert client.get("/api/v1/exports/certified", params={"store_id": denied["id"]}).status_code == 403
    query = client.post("/api/v1/certified-query", json={"sql": "select count(*) as rows from certified_sales"})
...
def test_supported_business_questions_return_real_rank_and_comparison(client):
    ...
    with TestingSession() as db:
        ...
        db.add_all([
            CertifiedAggregate(
                enterprise_id=enterprise_id, ingestion_run_id=run.id, store_id=store["id"], store_logical_id=store["logical_id"],
                ...
                source_definition_id=source["id"], source_logical_id=source["logical_id"], source_version=source["version"], model_version=1, model_checksum=registry_checksum(),
            ),
            CertifiedAggregate(
                enterprise_id=enterprise_id, ingestion_run_id=run.id, store_id=store["id"], store_logical_id=store["logical_id"],
